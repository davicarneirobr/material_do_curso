...
for (XX in 1:50){
  texto <- paste0("Esse aqui é o número ", XX)
  print(texto)
}
...
iniciando <- (7:31)
for (variable in iniciando) {
  print("Iniciando acao")
  
  sequencia <-(7:31)
  for (n in sequencia) {
    mensagem1 <- paste0("Calculando o quadrado de ", n)
    print(mensagem1)
    
    sequencia.quadrado <- (7:31)^2
    for (n2 in sequencia.quadrado) {
      mensagem2 <- paste0("O quadrado de", sequencia, "vale ", n2)
...
for (variavel in 7:31) {
  print("Iniciando a acao")
  print(paste0("Calculando o quadrado de ", variavel))
  variavel.ao.quadrado <- variavel^2
  print(paste0("Calculando o quadrado de ", variavel, " vale", variavel.ao.quadrado ))
}
...
arquivos <- c("dados/voos_de_janeiro.csv", "dados/voos_de_fevereiro.csv", "dados/voos_de_marco.csv", "dados/voos_de_abril.csv", "dados/voos_de_maio.csv", "dados/voos_de_junho.csv")
read_csv2(arquivos)



for (arquivo in arquivos) {
maior.atraso <- max(arquivo$atraso_saida, na.rm = TRUE)
texto <- paste0("O maior atraso foi de ", maior.atraso)
print(maior.atraso)
}

#2nd tentativa

arquivos.ate.junho <- c("dados/voos_de_janeiro.csv", "dados/voos_de_fevereiro.csv", "dados/voos_de_marco.csv", "dados/voos_de_abril.csv", "dados/voos_de_maio.csv", "dados/voos_de_junho.csv")
for (arquivo in arquivos.ate.junho) {
  dados <-  read_csv2(arquivo)
  maior.atraso <-  max (dados$atraso_saida, na.rm = TRUE)
  texto <- paste0("O maior atraso do arquivo ", "foi de ", maior.atraso, " minutos")
  print(texto)
}
...
for (arquivo in arquivos.ate.junho) {
  dados <-  read_csv2(arquivo)
  maior.atraso <-  max (dados$atraso_saida, na.rm = TRUE)
  mais.adiantado <- (min(dados$atraso_saida, na.rm = TRUE))*(-1)
  texto1 <- paste0("O voo com maior atraso atrasou ", maior.atraso, " minutos")
  texto2 <- paste0("O voo mais adiantado adiantou ", mais.adiantado, " minutos")
  print(texto1)
  print(texto2)
}

#funcoes para juntar linhas
