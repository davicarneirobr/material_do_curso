...
soma_de_1_a_30 <- 0 #o objeto tem que estar zerado
...
vetor.de.medias.destinos <-  NULL
lista.de.aeroportos.destino <- unique(base_de_dados$destino)

for (aeroporto in lista.de.aeroportos.destino) {
  print(paste0("Calculando media de chegada ao destino", aeroporto))
  print(paste0("Filtrando base..."))
  base.filtrada <- base_de_dados[base_de_dados$destino == aeroporto, ]
  print(paste0("Calculando medias de atraso na chegada..."))
  media.chegada <- mean(base.filtrada$atraso_chegada, na.rm=TRUE)
  print(paste0("A media de atraso dos voos que chegaram em ", aeroporto, " foi de ", media.chegada, " minutos."))
  print(paste0("Salvando em um vetor..."))
  vetor.de.medias.destinos <- c(vetor.de.medias.destinos, media.chegada)
  print(paste0("Vetor com a media do aeroporto ", aeroporto, " : ", vetor.de.medias.destinos))
}
...
vetor_de_atraso_na_chegada <- NULL

for (aeroporto_de_destino in lista_de_aeroportos_de_destino) {
  base_filtrada <- filter(base_de_dados, destino == aeroporto_de_destino)
  coluna_que_me_interessa <- base_filtrada$atraso_chegada
  media_de_atraso_na_chegada <- mean(coluna_que_me_interessa, na.rm=TRUE)
  vetor_de_atraso_na_chegada <- c(vetor_de_atraso_na_chegada, media_de_atraso_na_chegada)
  print(paste0("O vetor de atraso na chegada ", vetor_de_atraso_na_chegada))
}
...
tabela_de_resultados_chegada <- data.frame(
  aeroportos = lista_de_aeroportos_de_destino,
  atraso = vetor_de_atraso_na_chegada
)

min(vetor_de_atraso_na_chegada)
mean(vetor_de_atraso_na_chegada)
max(vetor_de_atraso_na_chegada)
